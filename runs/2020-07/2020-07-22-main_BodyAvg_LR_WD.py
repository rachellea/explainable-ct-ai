...
if __name__=='__main__':
    for learning_rate in [1e-3,1e-1,10]:
        for weight_decay in [0,1e-7,1e-3]:
            tot0 = timeit.default_timer()
            DukeCTExperiment(descriptor='BodyAvg_LR'+str(learning_rate)+'_WD'+str(weight_decay),
                        custom_net = custom_models_base.Body_Avg,
                        custom_net_args = {'n_outputs':83},
                        loss = 'bce', loss_args = {},
                        
                        learning_rate = learning_rate, #default 1e-3
                        weight_decay = weight_decay, #default 1e-7
