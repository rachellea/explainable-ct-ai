...
if __name__=='__main__':
    ...
    for which_scans_dict in [random_valid_scans.return_which_scans(), enc_val_scans.return_which_scans()]:
    
        for key in experiments.keys():
            params_path_choice = experiments[key]['params_path_choice']
            valid_results_dir = experiments[key]['valid_results_dir']
            
            for attention_type in ['disease-reps-avg']:
                tot0 = timeit.default_timer()
                run_attn_analysis.AttentionAnalysis(task=['blue_heatmaps'],
