...
if __name__=='__main__':
    ...
    #Note that this uses the SptialAttn4Mask loss but the actual model
    #is SpatialAttn5Mask which produces attention of the same shape but
    #produces the attention using a different mechanism.
    for nonlinearity in ['sigmoid','softmax']:
        DukeCTExperiment(descriptor='BodyDiseaseSpatialAttn5Mask-'+nonlinearity, #MODEL FIVE
                custom_net = custom_models_mask.BodyDiseaseSpatialAttn5Mask, #MODEL FIVE
                custom_net_args = {'n_outputs':81,'nonlinearity':nonlinearity},
                loss_string = 'BodyDiseaseSpatialAttn4Mask-loss', #LOSS FOR MODEL FOUR
