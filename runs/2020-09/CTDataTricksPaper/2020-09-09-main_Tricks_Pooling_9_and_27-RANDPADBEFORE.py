...
if __name__=='__main__':
    ...
    DukeCTExperiment(descriptor='Tricks-BodyAvg-9Projections-BASE-RANDPADBEFORE',
                custom_net = custom_models_projections.BodyAvg_Projected_RANDPADBEFORE,
                custom_net_args = {'n_outputs':83,'projection':9},
                loss_string = 'bce',
                dataset_args = merge_dicts(SHARED_DATASET_ARGS,
                                        {'view':'axial',
                                        'projections9':False, #NO PROJECTION***
                                        'data_augment':{'train':True,'valid':False,'test':False},
                                        'volume_prep_args':{
                                                'pixel_bounds':[-1000,800],
                                                'max_slices':405, #45 WHEN USING 9PROJECTIONS, 405 WHEN USING NO PROJECTION
    ...
    ### 27 Projections ###
    tot0 = timeit.default_timer()
    DukeCTExperiment(descriptor='Tricks-BodyAvg-27Projections-RANDPADBEFORE',
                custom_net = custom_models_projections.BodyAvg_Projected_RANDPADBEFORE,
                custom_net_args = {'n_outputs':83,'projection':27},
                loss_string = 'bce',
                dataset_args = merge_dicts(SHARED_DATASET_ARGS,
                                        {'view':'axial',
                                        'projections9':False, #NO PROJECTION***
